...
type Props = {
  ...
  /** Conversion still uses the flower (made/missed + kick location). */
  ...
  /** Single-tap actions: pass / line_break / try / negative_action (no zone/quality). */
...
function LivePenaltySubpanel({ yellowSecondsLeft, onYellowCardActivate, onSubmit }: PenaltySubpanelProps) {
  ...
  return (
    <div className="live-penalty-pick">
      <div className={`live-penalty-cards${yellowRowLocked ? ' live-penalty-cards-locked' : ''}`} role="group" aria-label="Discipline card (optional)">
        <button type="button" className={`live-penalty-card live-penalty-card-yc${card === 'yellow' ? ' live-penalty-card-pressed' : ''}`} aria-pressed={card === 'yellow'} disabled={yellowRowLocked} title="Yellow card" onClick={(e) => { e.stopPropagation(); if (yellowRowLocked) return; if (card === 'yellow') { setCard(null); onYellowCardActivate(false); return; } setCard('yellow'); onYellowCardActivate(true); }}>Yellow card</button>
        <button type="button" className={`live-penalty-card live-penalty-card-rc${card === 'red' ? ' live-penalty-card-pressed' : ''}`} aria-pressed={card === 'red'} disabled={yellowRowLocked} title="Red card" onClick={(e) => { e.stopPropagation(); if (yellowRowLocked) return; if (card === 'yellow') onYellowCardActivate(false); setCard((c) => (c === 'red' ? null : 'red')); }}>Red card</button>
      </div>
      {yellowRowLocked ? (
...
export function SimplePlayerActions({
  ...
  const startYellowSinBin = useCallback((playerId: string) => {
    setYellowSinBinUntilMs((prev) => ({ ...prev, [playerId]: Date.now() + YELLOW_CARD_COOLDOWN_SEC * 1000 }));
  }, []);
          ...
          <div className="live-opp-stat-board" aria-label="Substitutions and cards by team">
            <div className="live-opp-stat-stack">
              <section className="live-opp-stat-card" aria-labelledby="simple-opp-stat-subs">
                <h3 id="simple-opp-stat-subs" className="live-opp-stat-card-title">Subs</h3>
              ...
              <section className="live-opp-stat-card" aria-labelledby="simple-opp-stat-yc">
                <h3 id="simple-opp-stat-yc" className="live-opp-stat-card-title">YC</h3>
                <div className="live-opp-stat-line">
                  <span className="live-opp-stat-team" title={opponentStatBoard.themLabel}>{opponentStatBoard.themLabel}</span>
                  <span className="live-opp-stat-delta-group">
              ...
              <section className="live-opp-stat-card" aria-labelledby="simple-opp-stat-rc">
                <h3 id="simple-opp-stat-rc" className="live-opp-stat-card-title">RC</h3>
                <div className="live-opp-stat-line">
                  <span className="live-opp-stat-team" title={opponentStatBoard.themLabel}>{opponentStatBoard.themLabel}</span>
                  <span className="live-opp-stat-delta-group">
          ...
          {players.map((p) => {
            ...
            return (
              <div key={p.id} className={`on-field-player-block${rowSinBin ? ' on-field-player-block--sin-bin' : ''}`}>
                <div className={`on-field-row-shell${showSinBinRowOverlay ? ' on-field-row-shell--overlay' : ''}`}>
                  <div className={`on-field-row-inner${rowSinBin ? ' on-field-row-inner--sin-bin' : ''}`}>
                    ...
                    >
                      ...
                      <span className="on-field-name">
                        ...
                        <PlayerDisciplineSuffix badges={disciplineBadgesByPlayerId[p.id] ?? emptyDisciplineBadges()} />
                      </span>
                    </button>
                    <div className="on-field-round-actions">
                      {mode === 'attack' ? (
                        <>
                          {owesConversion ? (
                            <ZoneFlowerActionButton
                              ...
                              disabled={rowSinBin}
                              conversionKick={pendingConversionKick}
                              onAction={onAction}
                            />
                          ) : (
                            <>
                              <button type="button" className="live-action-round" disabled={rowSinBin} title="Pass" aria-label={`Pass — ${formatPlayerLabel(p)}`} onClick={(e) => tapThenBlur(e, () => onSimpleAction('pass', p.id))}>P</button>
                              <button type="button" className="live-action-round" disabled={rowSinBin} title="Offload" aria-label={`Offload — ${formatPlayerLabel(p)}`} onClick={(e) => tapThenBlur(e, () => onSimpleAction('offload', p.id))}>O</button>
                              <button type="button" className="live-action-round" disabled={rowSinBin} title="Line break" aria-label={`Line break — ${formatPlayerLabel(p)}`} onClick={(e) => tapThenBlur(e, () => onSimpleAction('line_break', p.id))}>LB</button>
                              <button type="button" className="live-action-round" disabled={rowSinBin} title="Try" aria-label={`Try — ${formatPlayerLabel(p)}`} onClick={(e) => tapThenBlur(e, () => onSimpleAction('try', p.id))}>Tr</button>
                              <button type="button" className="live-action-round live-action-round-negative" disabled={rowSinBin} title="Negative play" aria-label={`Negative — ${formatPlayerLabel(p)}`} onClick={(e) => tapThenBlur(e, () => onSimpleAction('negative_action', p.id))}>Neg</button>
                            </>
                          )}
                          <button
                            type="button"
                        ...
                        <>
                          <button type="button" className="live-action-round" disabled={rowSinBin} title="Tackle made" aria-label={`Tackle made — ${formatPlayerLabel(p)}`} onClick={(e) => tapThenBlur(e, () => onSimpleTackle(p.id, 'made'))}>M</button>
                  ...
                  {showSinBinRowOverlay ? (
                    <button
                      ...
                      aria-live="polite"
                      aria-label={sinBinSec > 0 ? `Return to play, ${formatMmSs(sinBinSec)} remaining.` : 'Return to play timer finished. Tap to confirm.'}
                      onClick={(e) => { e.stopPropagation(); clearYellowSinBin(p.id); }}
                    >
                      <div className="live-penalty-yellow-bin live-penalty-yellow-bin--row">
                        <div className="live-penalty-yellow-bin-text">
                          <span className="live-penalty-yellow-bin-label">Return to play</span>
                          {sinBinSec === 0 ? <span className="on-field-sin-bin-hint muted">Tap when player may return</span> : null}
                        </div>
                        <span className="live-penalty-yellow-bin-time" title="Time remaining">{formatMmSs(sinBinSec)}</span>
                      </div>
                    </button>
                  ) : null}
                </div>
                  ...
                  <div className="on-field-sub-pick" role="group" aria-label="Choose player coming on">
                    <div className="on-field-row-stats" aria-label="Player stats">
                      <div className="on-field-row-stat">
                        ...
                        <span className="on-field-row-stat-value">{formatPlayerMinutesLabel(getPlayerMinutesMs(p.id))}</span>
                      </div>
                    </div>
                    ...
                    {subsSorted.length === 0 ? (
                      ...
                      <div className="on-field-sub-pick-chips">
                        {subsSorted.map((c) => (
                          <button key={c.id} type="button" className="on-field-sub-pick-btn" onClick={(e) => { e.stopPropagation(); onSubstitute(p.id, c.id); setSubPickerFor(null); }}>
                            <span className="on-field-sub-pick-btn-text">
                              {formatPlayerLabel(c)}
                              <PlayerDisciplineSuffix badges={disciplineBadgesByPlayerId[c.id] ?? emptyDisciplineBadges()} />
                            </span>
                            <span className="on-field-sub-pick-btn-mins muted">{formatPlayerMinutesLabel(getPlayerMinutesMs(c.id))}</span>
                          </button>
                        ))}
                      </div>
                    )}
